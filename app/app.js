...
const app = {
  ...
  createTetromino: (arg) => {
    ...
    for (let i = 0; i < 4; i++) {
      if (i != 0) {
        whereToDraw = {
          x: arg.origin.x + arg.shape[i - 1].x,
          y: arg.origin.y + arg.shape[i - 1].y,
        };
        ctx.fillRect(whereToDraw.x, whereToDraw.y, 25, 25);
      } else {
        // Quand i est a 0, il dessine sur le canva le bloc d'origine ayant déjà les coordonnées défini dans chaque instance au sein de la propriété origin
  ...
  createGrid: () => {
    ...
    for (let i = 0; i < grid.length; i++) {
      // Boucle sur les 22 elements vides de l'array
      grid[i] = new Array(10); // Création dans chaque element d'un nouveau array contenant 10 élements vides
    }

    for (let i = 0; i < 10; i++) {
      // Création d'une boucle sur les colonnes de grid
      for (let j = 0; j < 22; j++) {
        // Et sur les cellules qui les compose
